...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!NOVICE_ROLE_ID || !ADVANCED_ROLE_ID) {
      return interaction.reply({ content: '❌ 역할 ID 환경 변수(NOVICE_ROLE_ID/ADVANCED_ROLE_ID)가 설정되지 않았습니다.', flags: MessageFlags.Ephemeral });
    }
    ...
    for (const m of interaction.guild.members.cache.values()) {
      if (m.roles.cache.has(NOVICE_ROLE_ID)) members.push({ id: m.id, name: m.displayName, division: 'novice' });
      else if (m.roles.cache.has(ADVANCED_ROLE_ID)) members.push({ id: m.id, name: m.displayName, division: 'advanced' });
    }

    const { added, updated } = await syncRoster(members);
    await interaction.editReply({ content: `✅ 명단 동기화 완료: 대상 ${members.length}명 (추가 ${added}, 갱신 ${updated})` });
  }
};
